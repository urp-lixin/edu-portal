  ...
  def load(className: String, classLoader: ClassLoader = null): Class[_] = {
    ...
    if (buildins.contains(className)) {
      ...
      if (className.startsWith(": ")) {
        val clzName = className.substring(2)
        if (buildins.contains(clzName)) {
          buildins(clzName)
        } else {
          loader.loadClass("java.lang." + className.substring(2))
        }
      } else {
    ...
    if (buildins.contains(className)) {
      ...
      if (null != loader.getResource(Strings.replace(className, ".", "/") + ".class")) {
        if (className.startsWith(": ")) {
          val clzName = className.substring(2)
          if (buildins.contains(clzName)) {
            buildins.get(clzName)
          } else {
            Some(loader.loadClass("java.lang." + className.substring(2)))
          }
        } else {
